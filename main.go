...
func main() {
	...
	targetExpr := `$.data[0].firstName`
	mapping(sourceObj, targetObj, sourceExpr, targetExpr)
}
func mapping(sourceObj string, targetObj string, sourceExpr string, targetExpr string) {
	...
	if err != nil {
		fmt.Println("Error evaluating data JSONPath expression:", err)
		fmt.Printf("Error evaluating data JSONPath expression: %v\nExpression: %v\nObject: %v", err.Error(), targetExpr, targetObj)
		return
	}
	fmt.Printf("oldValue: %v\n", oldValue)

	dataResult, err := updateField(dataMap, "$.data[0].firstName", nameResult)
	if err != nil {
